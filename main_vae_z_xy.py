...
def plot_x(model,plot,sample_x,sample_y,i,n_sample,path):
    fig = plt.figure(figsize=(12, 8))
    n_y = 10

    y = np.eye(n_y).astype(np.float32)
    sample_z = model.q_sample_mean_x(sample_x,sample_y)
    X = np.array([[np.array(model.p_sample_mean_x(_z[np.newaxis],
                                                  y[_y][np.newaxis]))[0]
...
def train(data, activation, plot_image, rseed, n_epoch, Optimizer, l, k, sample_l, sample_k, alpha, sampling_type, n_batch, annealing, annealing_epoch, bn_layer, options_dict):
    ...
    if data == "mnist":
        ...
        q0_0 = bn(DenseLayer(y_in,num_units=512,nonlinearity=activation))
        q_0  = bn(DenseLayer(q_in,num_units=512,nonlinearity=activation))
        q_1  = bn(DenseLayer(ConcatLayer([q_0,q0_0]),num_units=512,nonlinearity=activation))
        q_mean = DenseLayer(q_1,num_units=n_z,nonlinearity=linear)
        q_var = DenseLayer(q_1,num_units=n_z,nonlinearity=softplus)
        q = Gaussian(q_mean,q_var,given=[q_in,y_in])

        p0_0 = bn(DenseLayer(y_in,num_units=512,nonlinearity=activation))
        p_0  = bn(DenseLayer(ConcatLayer([p_in,p0_0]),num_units=512,nonlinearity=activation))
        p_1  = bn(DenseLayer(p_0,num_units=512,nonlinearity=activation))
        p_mean = DenseLayer(p_1,num_units=n_x,nonlinearity=sigmoid)
        p = Bernoulli(p_mean,given=[p_in,y_in])
    ...
    if alpha == None:
        ...
        model = VAE(q,p,n_batch,Optimizer,l=l,k=k,alpha=alpha,random=rseed)
    ...
    for i in range(1, n_epoch + 1):
        ...
        if ((i%100 == 0) or (i == 1)) and (plot_image is True):
            log_likelihood_test = model.log_likelihood_test(
                [test_x,test_y], k=1, l=1, mode=sampling_type)
            log_likelihood_test_5000 = model.log_likelihood_test(
                [test_x,test_y], n_batch=10, k=sample_k, l=sample_l, mode=sampling_type)
            if alpha == None:
                lw = "epoch = %d, lower bound (train) = %lf (%lf %lf) lower bound (test) = %lf lower bound (test 5000) = %lf\n" % (i, sum(lowerbound_train), lowerbound_train[0], lowerbound_train[1], np.mean(log_likelihood_test), np.mean(log_likelihood_test_5000))
            else:
            ...
            plot_x(model,plot,sample_x,sample_y,i,n_sample,dirpath)
