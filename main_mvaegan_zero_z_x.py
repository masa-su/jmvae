...
from Tars.model import MVAEGAN_OLD
...
def train(data, activation, plot_image, rseed, n_epoch, Optimizer,
          l, k, sample_l, sample_k, gan_gamma,
    ...
    model = MVAEGAN_OLD(q, p, d, n_batch, Optimizer, l=l,
                    random=rseed, gan_gamma=gan_gamma)
    ...
    options_dict["file"] = __file__
    with open(os.path.join(dirpath, "paramaters.dump"), "w") as f:
    ...
    for i in range(1, n_epoch + 1):
        ...
        if ((i % 10 == 0) or (i == 1)) and (plot_image is True):
            ...
            lw["loss (train) [0]"] = lowerbound_train[3]
            lw["loss (train) [1]"] = lowerbound_train[4]
